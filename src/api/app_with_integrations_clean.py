...
def initialize_bot():
    ...
    global faq_bot, slack_integration, email_integration, initialization_status, slack_processor
    ...
    try:
        ...
        initialization_status = {"status": "ready", "message": "Enterprise bot ready!"}
        logger.info("🎓 Enterprise HR Policy Bot initialized successfully!")
        
    except Exception as e:
        logger.error(f"❌ Failed to initialize enterprise bot: {e}")
        initialization_status = {"status": "error", "message": f"Enterprise initialization failed: {str(e)}"}
...
def ask_question():
    ...
    try:
        # Use fast_answer for immediate response with web platform specified
        result = faq_bot.fast_answer(question, platform="web")
...
def _get_bot_response_sync(question: str) -> dict:
    """Synchronous wrapper for bot's async ask_question method"""
    import asyncio
    ...
    try:
        ...
        if loop.is_running():
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(asyncio.run, get_answer())
...
def slack_events():
    ...
    if not slack_processor:
        return jsonify({"error": "Enterprise Slack processor not available"}), 503
    ...
    if data.get("type") == "event_callback":
        try:
            result = slack_processor.handle_slack_event(data)
            return jsonify(result)
        except Exception as e:
            logger.error(f"❌ Enterprise Slack event handling failed: {e}")
            return jsonify({"status": "error", "message": "Enterprise processing failed"}), 500
    
    # All other event types
    return jsonify({"status": "ok", "message": "Event type not processed"})
...
def health_check():
    ...
    return jsonify({
        ...
        "bot_status": initialization_status["status"],
        "bot_message": initialization_status["message"]
    })
...
<body>
    ...
    <script>
        ...
        async function checkStatus() {
            try {
                ...
                if (data.bot.status === 'ready') {
                    document.getElementById('botStatus').className = 'status-dot';
                    document.getElementById('botStatusText').textContent = 'Bot Ready';
                    document.getElementById('statusBadge').textContent = 'Ready';
                    document.getElementById('statusBadge').style.background = '#e8f5e8';
                    document.getElementById('statusBadge').style.color = '#2e7d2e';
                } else {
                    document.getElementById('botStatus').className = 'status-dot offline';
                    document.getElementById('botStatusText').textContent = 'Bot Initializing...';
                    document.getElementById('statusBadge').textContent = 'Initializing...';
                    document.getElementById('statusBadge').style.background = '#fff3e0';
                    document.getElementById('statusBadge').style.color = '#f57c00';
                }
                
                // Update integration status
                document.getElementById('slackStatus').className = 
                    data.integrations.slack ? 'status-dot' : 'status-dot offline';
                document.getElementById('emailStatus').className = 
                    data.integrations.email ? 'status-dot' : 'status-dot offline';
                    
            } catch (error) {
                console.error('Error checking status:', error);
                document.getElementById('botStatus').className = 'status-dot offline';
                document.getElementById('botStatusText').textContent = 'Connection Error';
                document.getElementById('statusBadge').textContent = 'Offline';
                document.getElementById('statusBadge').style.background = '#ffebee';
                document.getElementById('statusBadge').style.color = '#d32f2f';
            }
        }
        ...
        function convertMarkdownToHtml(content) {
            ...
            if (content.includes('<p>') || content.includes('<h1>') || content.includes('<h2>') || content.includes('<h3>') || content.includes('<ul>') || content.includes('<li>')) {
                // Content is already HTML, return as-is
                return content;
            }
            
            // If it's still markdown, convert it
            let html = content;
            
            // Headers
            html = html.replace(/^## (.+)$/gm, '<h2 style="font-size: 18px; font-weight: bold; margin: 15px 0 10px 0; color: #333;">$1</h2>');
            html = html.replace(/^### (.+)$/gm, '<h3 style="font-size: 16px; font-weight: bold; margin: 12px 0 8px 0; color: #444;">$1</h3>');
            html = html.replace(/^# (.+)$/gm, '<h1 style="font-size: 20px; font-weight: bold; margin: 18px 0 12px 0; color: #222;">$1</h1>');
            ...
            // Horizontal rules
            html = html.replace(/^={3,}.*$/gm, '<hr style="margin: 15px 0; border: none; border-top: 2px solid #ddd;">');
            html = html.replace(/^---$/gm, '<hr style="margin: 15px 0; border: none; border-top: 1px solid #ddd;">');
            
            // Convert double line breaks to paragraph breaks
            html = html.replace(/\n\n/g, '</p><p style="margin: 10px 0;">');
            
            // Wrap in paragraph if not already wrapped and doesn't have HTML elements
            if (!html.includes('<p>') && !html.includes('<h1>') && !html.includes('<h2>') && !html.includes('<h3>') && !html.includes('<ul>') && !html.includes('<hr>')) {
...
if __name__ == "__main__":
    ...
    # Start Flask app immediately - don't wait for bot initialization
    ...
    # Start bot initialization in background AFTER starting Flask
    init_thread = Thread(target=initialize_bot)
    init_thread.daemon = True
    init_thread.start()
    
    # Start Flask immediately - this is what Render needs to detect
